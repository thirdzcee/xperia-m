...
static void wpas_p2p_ie_update(void *ctx, struct wpabuf *beacon_ies,
			       struct wpabuf *proberesp_ies, struct wpabuf *assocresp_ies)  //CONN-FY-FixWFD_5.1.3
{
	struct wpa_supplicant *wpa_s = ctx;
	if (wpa_s->ap_iface) {
		...
		if (!(hapd->conf->p2p & P2P_GROUP_OWNER)) {
			...
			wpabuf_free(assocresp_ies); //CONN-FY-FixWFD_5.1.3 
			return;
		}
		if (beacon_ies) {
...
#ifdef CONFIG_WFD
		wpabuf_free(hapd->wfd_assoc_resp_ie);   //CONN-FY-FixWFD_5.1.3 
		hapd->wfd_assoc_resp_ie = assocresp_ies;   //CONN-FY-FixWFD_5.1.3 
#endif
	} else {
		wpabuf_free(beacon_ies);
		wpabuf_free(proberesp_ies);
